...
const MoviesPage: React.FC = () => {
  ...
  // Fetch movies by genre or popular if no genre selected
  const { 
    data: moviesData, 
    isLoading: isMoviesLoading,
    isFetching
  } = useQuery(
    ['movies', selectedGenre, currentPage],
    () => selectedGenre 
      ? tmdbService.getMoviesByGenre(selectedGenre, currentPage)
      : tmdbService.getPopular('movie').then(results => ({ results, total_pages: 500 }))
  );
  ...
  if (isGenresLoading || (!genres && isMoviesLoading)) {
    return <LoadingPage />;
  }
